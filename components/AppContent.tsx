...
export default function AppContent() {
  ...
  const [guideSection, setGuideSection] = useState('checklist')
  ...
  const [expForm, setExpForm] = useState({ date: new Date().toISOString().slice(0,10), category: 'van', amount: '', note: '' })
  ...
  const switchTab  = (tb) => { setViewHistory([]); setTab(tb); setView('list'); setSelDoc(null); setSelPass(null); setSelAddr(null); setSelResume(null); setSearch('') }
  ...
  // ── FINANCE HANDLERS ─────────────────────────────────
  const EXP_CATS = [
    { id:'van',       label:'Van Rental',     icon:'🚐' },
    { id:'fuel',      label:'Fuel',           icon:'⛽' },
    { id:'phone',     label:'Phone/Data',     icon:'📱' },
    { id:'insurance', label:'Insurance',      icon:'🛡️' },
    { id:'clothing',  label:'Work Clothing',  icon:'👕' },
...
<script>window.onload = function(){ window.print() }</script>
  ...
  const addMileage = async () => {
    if (!user || !mileForm.miles || !mileForm.date) return
    const miles = parseFloat(mileForm.miles)
    // Calculate mileage allowance: 45p first 10k miles, 25p after
    const yearMiles = finance.filter(e => e.category === 'mileage' && e.date.startsWith(new Date().getFullYear().toString())).reduce((s,e) => s + Number(e.note.split('|')[0] || 0), 0)
    const rate = yearMiles < 10000 ? (yearMiles + miles <= 10000 ? 0.45 : ((10000 - yearMiles) * 0.45 + (miles - (10000 - yearMiles)) * 0.25) / miles) : 0.25
    const amount = miles * (yearMiles < 10000 ? (yearMiles + miles <= 10000 ? 0.45 : ((10000 - yearMiles) * 0.45 + (miles - (10000 - yearMiles)) * 0.25) / miles) : 0.25)
    const entry = await DB.addFinance(user.id, 'expense', mileForm.date, parseFloat(amount.toFixed(2)), 'mileage', miles + '|' + mileForm.note)
    if (entry) setFinance(prev => [entry, ...prev])
    setMileForm({ date: new Date().toISOString().slice(0,10), miles: '', note: '' })
  }

  const getTotalYearMiles = () => finance.filter(e => e.category === 'mileage' && e.date.startsWith(new Date().getFullYear().toString())).reduce((s,e) => s + Number(e.note.split('|')[0] || 0), 0)

  // HMRC Deadlines
  const getHMRCDeadlines = () => {
    const now = new Date()
    const y = now.getFullYear()
    const deadlines = [
      { date: new Date(y, 3, 5),   label: 'End of Tax Year',              icon: '📅', detail: `Tax year ${y-1}/${y} closes` },
      { date: new Date(y, 9, 5),   label: 'Register Self Assessment',      icon: '📝', detail: 'Deadline to register for Self Assessment' },
      { date: new Date(y, 9, 31),  label: 'Paper Self Assessment',         icon: '📮', detail: `Paper return for ${y-1}/${y}` },
      { date: new Date(y, 0, 31),  label: 'Online Self Assessment',        icon: '💻', detail: `Online return + payment for ${y-1}/${y}` },
      { date: new Date(y, 6, 31),  label: 'Payment on Account (2nd)',      icon: '💰', detail: '2nd advance payment to HMRC' },
      { date: new Date(y+1, 0, 31),label: 'Next Self Assessment',          icon: '💻', detail: `Online return + payment for ${y}/${y+1}` },
    ]
    return deadlines.map(d => {
      const diff = Math.ceil((d.date - now) / (1000*60*60*24))
      return { ...d, diff, passed: diff < 0 }
    }).filter(d => d.diff > -30).sort((a,b) => a.diff - b.diff)
  }

  const addIncome = async () => {
    if (!user || !incForm.amount || !incForm.date) return
    const entry = await DB.addFinance(user.id, 'income', incForm.date, parseFloat(incForm.amount), 'income', incForm.note)
    if (entry) setFinance(prev => [entry, ...prev])
    setIncForm({ date: new Date().toISOString().slice(0,10), amount: '', note: '' })
  }

  const addExpense = async () => {
    if (!user || !expForm.amount || !expForm.date) return
    const entry = await DB.addFinance(user.id, 'expense', expForm.date, parseFloat(expForm.amount), expForm.category, expForm.note)
    if (entry) setFinance(prev => [entry, ...prev])
    setExpForm({ date: new Date().toISOString().slice(0,10), category: 'van', amount: '', note: '' })
  }

  const deleteFinEntry = async (id) => {
  ...
  return (
    <div style={{ minHeight: '100vh', background: theme === 'dark' ? '#0f172a' : C.bg, color: theme === 'dark' ? '#f1f5f9' : C.text }}>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '16px 16px 120px' }}>
        ...
        {tab === 'finance' && (
          <FinanceTab
            finance={finance} finTab={finTab} setFinTab={setFinTab}
            finMonth={finMonth} setFinMonth={setFinMonth}
            incForm={incForm} setIncForm={setIncForm}
            expForm={expForm} setExpForm={setExpForm}
            mileForm={mileForm} setMileForm={setMileForm}
            addIncome={addIncome} addExpense={addExpense}
            addMileage={addMileage} deleteFinEntry={deleteFinEntry}
            generateCSV={generateCSV} generatePDFReport={generatePDFReport}
            fmtGBP={fmtGBP} finMonthLabel={finMonthLabel}
            EXP_CATS={EXP_CATS} getTotalYearMiles={getTotalYearMiles}
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: dark ? '#1e293b' : '#fff', borderTop: `1px solid ${D.border}`, boxShadow: '0 -4px 20px rgba(0,0,0,0.15)', zIndex: 90, display: 'flex', paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <NavBtn id="home"    icon="🏠" en="Home"    ru="Головна" uk="Головна"   tab={tab} dark={dark} lang={lang} switchTab={switchTab} />
          ...
          <NavBtn id="finance" icon="💷" en="Finance" ru="Фінанси" uk="Фінанси" tab={tab} dark={dark} lang={lang} switchTab={switchTab} />
