...
export default function Page() {
  ...
  const switchLang = () => { const nl = lang === 'ru' ? 'en' : 'ru'; setLang(nl); localStorage.setItem('uk_lang', nl) }
  ...
  if (photoView) return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.96)', zIndex: 999, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }} onClick={() => setPhotoView(null)}>
      ...
      <div style={{ color: 'rgba(255,255,255,0.4)', fontSize: 13, marginTop: 16 }}>{t('Tap to close', 'Нажми чтобы закрыть')}</div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyM})`, position: 'sticky', top: 0, zIndex: 100, boxShadow: '0 2px 20px rgba(0,0,0,0.25)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, height: 56 }}>
            ...
            <button onClick={switchLang} style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 8, padding: '4px 10px', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{lang === 'ru' ? 'EN' : 'RU'}</button>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '16px 16px 100px' }}>
          ...
          <>
            {expiring60.length > 0 && (
              <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 12, padding: '12px 16px', marginBottom: 12 }}>
                ...
                {expiring60.map(d => <div key={d.id} style={{ fontSize: 12, color: '#7c2d12', marginBottom: 2 }}>· {lang === 'ru' && d.title_ru ? d.title_ru : d.title} — {daysUntil(d.valid_until ?? '')}d</div>)}
              </div>
            )}
            <div style={{ position: 'relative', marginBottom: 14 }}>
              <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: C.muted }}>🔍</span>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('Search…', 'Поиск…')} style={{ ...inputStyle, paddingLeft: 36 }} />
            ...
            {pinned.length > 0 && (<><SLabel>📌 {t('Pinned', 'Закреплённые')}</SLabel>{pinned.map(d => <DocCard key={d.id} doc={d} cat={cat(d.category)} lang={lang} onOpen={() => { setSelDoc(d); setView('detail') }} />)}<div style={{ height: 8 }} /></>)}
            {rest.map(d => <DocCard key={d.id} doc={d} cat={cat(d.category)} lang={lang} onOpen={() => { setSelDoc(d); setView('detail') }} />)}

            {filteredDocs.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', color: C.muted }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>📂</div>
                <div>{t('No documents', 'Нет документов')}</div>
              </div>
            )}
        ...
        {tab === 'docs' && view === 'detail' && selDoc && (() => {
          ...
          return (<>
            <button onClick={() => { setView('list'); setSelDoc(null) }} style={{ background: 'none', border: 'none', color: C.blue, fontSize: 14, fontWeight: 600, cursor: 'pointer', padding: '0 0 16px', display: 'flex', alignItems: 'center', gap: 5 }}>← {t('Back', 'Назад')}</button>
            <div style={{ background: `linear-gradient(135deg, ${c.color}, ${c.color}bb)`, borderRadius: 20, padding: '24px 24px 20px', marginBottom: 3, color: '#fff', boxShadow: `0 8px 30px ${c.color}40` }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 700 }}>{lang === 'ru' && selDoc.title_ru ? selDoc.title_ru : selDoc.title}</div>
            </div>
            <div style={{ background: C.surface, borderRadius: '0 0 20px 20px', padding: '20px 22px', marginBottom: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
              {selDoc.number && <DRow label={t('Number / Code', 'Номер / Код')}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}><span style={{ fontFamily: 'monospace', fontSize: 22, fontWeight: 800, letterSpacing: '0.07em', color: C.navy }}>{selDoc.number}</span><CopyBtn value={selDoc.number} lang={lang} /></div></DRow>}
              {selDoc.valid_from  && <DRow label={t('Valid from', 'Действует с')}>{formatDate(selDoc.valid_from)}</DRow>}
              {selDoc.valid_until && <DRow label={t('Valid until', 'Действует до')}><span>{formatDate(selDoc.valid_until)}</span><ExpiryBadge d={selDoc.valid_until} /></DRow>}
            ...
            {confirmDel === selDoc.id && <div style={{ background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: 14, padding: 18, textAlign: 'center' as const }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={() => handleDeleteDoc(selDoc.id)} style={{ flex: 1, background: C.red, border: 'none', borderRadius: 8, padding: 10, cursor: 'pointer', color: '#fff', fontSize: 13, fontWeight: 700 }}>{t('Delete', 'Удалить')}</button>
              </div>
            </div>}
          </>)
        })()}
        ...
        {tab === 'passport' && view === 'addPassport' && (
          <div style={{ background: C.surface, borderRadius: 16, padding: 24 }}>
            ...
            <FField label={t('Issued by', 'Кем выдан')}><input value={passForm.issued_by} onChange={e => setPassForm(f => ({ ...f, issued_by: e.target.value }))} placeholder={t('Ministry of Internal Affairs', 'МВД Украины')} style={inputStyle} /></FField>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <FField label={t('Issued', 'Дата выдачи')}><input type="date" value={passForm.issued_date} onChange={e => setPassForm(f => ({ ...f, issued_date: e.target.value }))} style={inputStyle} /></FField>
              <FField label={t('Expires', 'Срок действия')}><input type="date" value={passForm.expiry_date} onChange={e => setPassForm(f => ({ ...f, expiry_date: e.target.value }))} style={inputStyle} /></FField>
            </div>
            <FField label={t('Notes', 'Заметки')}><textarea value={passForm.notes} onChange={e => setPassForm(f => ({ ...f, notes: e.target.value }))} rows={2} style={{ ...inputStyle, resize: 'vertical' as const }} /></FField>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => switchTab('passport')} style={{ flex: 1, background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: 14, fontSize: 14, cursor: 'pointer', color: C.textSub, fontWeight: 600 }}>{t('Cancel', 'Отмена')}</button>
              <button onClick={handleAddPassport} disabled={!passForm.number || saving} style={{ flex: 2, background: passForm.number ? '#0369a1' : '#cbd5e0', border: 'none', borderRadius: 10, padding: 14, fontSize: 14, cursor: passForm.number ? 'pointer' : 'not-allowed', color: '#fff', fontWeight: 700 }}>{saving ? '⏳' : t('Save', 'Сохранить')}</button>
            </div>
          </div>
        )}
        ...
        {tab === 'passport' && view === 'passportDetail' && selPass && (
          <>
            <button onClick={() => { setView('list'); setSelPass(null) }} style={{ background: 'none', border: 'none', color: C.blue, fontSize: 14, fontWeight: 600, cursor: 'pointer', padding: '0 0 16px', display: 'flex', alignItems: 'center', gap: 5 }}>← {t('Back', 'Назад')}</button>
            ...
            <div style={{ background: C.surface, borderRadius: '0 0 20px 20px', padding: '20px 22px', marginBottom: 12 }}>
              {selPass.issued_by   && <DRow label={t('Issued by', 'Кем выдан')}>{selPass.issued_by}</DRow>}
              {selPass.issued_date && <DRow label={t('Issued', 'Дата выдачи')}>{formatDate(selPass.issued_date)}</DRow>}
            ...
            <div style={{ background: C.surface, borderRadius: 16, padding: '18px 20px', marginBottom: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.navy, marginBottom: 14 }}>📸 {t('Document Photos', 'Фото страниц')} ({selPass.passport_photos.length})</div>
              ...
              <div style={{ background: C.bg, borderRadius: 12, padding: 14 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: C.textSub, marginBottom: 8 }}>{t('Add page photo', 'Добавить фото страницы')}</div>
                ...
                <button onClick={() => passPhotoRef.current?.click()} disabled={saving} style={{ width: '100%', background: saving ? '#94a3b8' : '#0369a1', color: '#fff', border: 'none', borderRadius: 10, padding: '11px 14px', fontSize: 13, fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer' }}>
                  {saving ? '⏳ Uploading…' : `📷 ${t('Take / Choose Photo', 'Сфотографировать / Выбрать')}`}
                </button>
              </div>
            </div>

            <button onClick={() => setConfirmDel(selPass.id)} style={{ width: '100%', background: '#fee2e2', border: '1.5px solid #fca5a5', borderRadius: 10, padding: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', color: C.red }}>🗑 {t('Delete Passport', 'Удалить паспорт')}</button>
            {confirmDel === selPass.id && (
              <div style={{ background: '#fee2e2', borderRadius: 12, padding: 16, marginTop: 10, textAlign: 'center' as const }}>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setConfirmDel(null)} style={{ flex: 1, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, cursor: 'pointer', fontSize: 13 }}>{t('Cancel', 'Отмена')}</button>
                  <button onClick={() => handleDeletePassport(selPass.id)} style={{ flex: 1, background: C.red, border: 'none', borderRadius: 8, padding: 10, cursor: 'pointer', color: '#fff', fontSize: 13, fontWeight: 700 }}>{t('Delete', 'Удалить')}</button>
                </div>
              </div>
            )}
          </>
          ...
          <>
            <div style={{ background: `linear-gradient(135deg, ${C.navy}, ${C.navyM})`, borderRadius: 20, padding: '28px 24px 24px', marginBottom: 12, color: '#fff', textAlign: 'center' as const }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 20, marginTop: 14 }}>
                {profile?.dob && <div style={{ textAlign: 'center' as const }}><div style={{ fontSize: 11, opacity: 0.5 }}>{t('DOB', 'Дата рождения')}</div><div style={{ fontWeight: 600, fontSize: 14 }}>{formatDate(profile.dob)}</div></div>}
                {profile?.nationality && <div style={{ textAlign: 'center' as const }}><div style={{ fontSize: 11, opacity: 0.5 }}>{t('Nationality', 'Гражданство')}</div><div style={{ fontWeight: 600, fontSize: 14 }}>{profile.nationality}</div></div>}
              </div>
              <div style={{ marginTop: 12, fontSize: 12, opacity: 0.4 }}>{user?.email}</div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
              <button onClick={() => { setProfForm(profile ?? {}); setView('editProfile') }} style={{ background: C.surface, border: `1.5px solid ${C.border}`, borderRadius: 12, padding: 14, fontSize: 14, fontWeight: 600, cursor: 'pointer', color: C.navy }}>✏️ {t('Edit Profile', 'Редактировать')}</button>
              <button onClick={signOut} style={{ background: '#fee2e2', border: '1.5px solid #fca5a5', borderRadius: 12, padding: 14, fontSize: 14, fontWeight: 600, cursor: 'pointer', color: C.red }}>🚪 {t('Sign Out', 'Выйти')}</button>
            </div>

            <div style={{ background: C.surface, borderRadius: 14, padding: '16px 18px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.navy, marginBottom: 12 }}>🔒 {t('Privacy & Security', 'Конфиденциальность')}</div>
              {[
        ...
        {tab === 'profile' && view === 'editProfile' && (
          <div style={{ background: C.surface, borderRadius: 16, padding: 24 }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: C.navy, marginBottom: 20 }}>✏️ {t('Edit Profile', 'Редактировать профиль')}</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setView('list')} style={{ flex: 1, background: C.bg, border: `1.5px solid ${C.border}`, borderRadius: 10, padding: 14, fontSize: 14, cursor: 'pointer', color: C.textSub, fontWeight: 600 }}>{t('Cancel', 'Отмена')}</button>
              <button onClick={handleSaveProfile} disabled={saving} style={{ flex: 2, background: saving ? '#94a3b8' : C.navy, border: 'none', borderRadius: 10, padding: 14, fontSize: 14, cursor: 'pointer', color: '#fff', fontWeight: 700 }}>{saving ? '⏳' : t('Save', 'Сохранить')}</button>
            </div>
          </div>
        )}
  ...
  return (
    <div onClick={onOpen} style={{ background: '#fff', borderRadius: 14, marginBottom: 8, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', boxShadow: '0 1px 4px rgba(15,31,61,0.06)', borderLeft: `4px solid ${cat.color}` }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {doc.valid_until && <div style={{ fontSize: 11, color: '#a0aec0', marginTop: 2 }}>{lang === 'ru' ? 'до' : 'until'} {formatDate(doc.valid_until)}</div>}
      </div>
      <div style={{ color: '#cbd5e0', fontSize: 20 }}>›</div>
    </div>
  )
